...
- Legacy helper code for other providers may still exist in the repo, but the active reply path is Opus-only
...
- Ranking/list/count questions are intentionally stricter than ordinary latest-info questions
...
Latest local commit that introduced the main hardening:
...
- chart/ranking questions became stricter, but the hardening applies to `news`, `music`, and `web` generally

### Persistent typing refresh

Latest local commit for typing pacing:
...
2. Run `python -m py_compile wa_agent.py` before deployment or wider testing
3. Verify the target behavior with small direct probes before touching production

## Git Note

- This workspace should stay isolated from unrelated remotes
